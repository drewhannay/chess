...
public class TurnKeeper_Given_DifferentTurnCountWithTwoTeams_Should {
    ...
    public void setup() {
        ...
        int[] turnCounts = new int[]{1, 2};
        mTarget = TurnKeeper.createDifferentTurnCount(teamIds, turnCounts);
        // Expected order:
    ...
    @Test
    public void returnTeamTwoAfter2Turns() {
        finishTurns(2);
        assertEquals(Piece.TEAM_TWO, mTarget.getActiveTeamId());
    }
    ...
    @Test
    public void returnTeamOne1TurnAnd1Undo() {
        finishTurns(1);
        undoFinishTurns(1);
        assertEquals(Piece.TEAM_ONE, mTarget.getActiveTeamId());
    }
